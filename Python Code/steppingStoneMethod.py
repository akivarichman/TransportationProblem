    ...
    while stack:
        ...
        for col in range(columns):
            if col != j and (i, col) in basic_cells and (i, col) not in visited:
                ...
                stack.append(((i, col), new_path, visited.copy())) # sending the set 'visited' into the stack would send a reference, we need to copy the set to avoid future changes affecting this piece of the stack
    ...
    while True:
        ...
        for i in range(rows):
            for j in range(columns):
                if (i, j) not in basic_cells: # only find path and calculate opportunity cost for non-basic cells
